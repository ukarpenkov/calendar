...
Документ фиксирует общий порядок разработки приложения на основе требований проекта и потока данных `JSON -> SQLite -> UI`. **Визуальный эталон на текущем этапе — уже реализованный UI в приложении**, а не макеты в `pencil-new.pen`: при расхождении сохраняем и развиваем то, что в продукте, Pencil используем как вспомогательную опору и архив идей.
...
Канон интерфейса — **текущее состояние приложения** (типографика, отступы, цвета, композиция экранов). Файл `pencil-new.pen` не является источником истины: узлы ниже — карта соответствий для навигации по макету и обсуждений, без обязанности подгонять реализацию под Pencil.
...
1. Сначала формализуем контракт календарных данных, чтобы все последующие слои опирались на единый и предсказуемый формат.
2. Затем собираем общий слой валидации и нормализации, чтобы входящий `JSON` приводился к строгой доменной модели до попадания в UI и хранилище.
3. После этого поднимаем `SQLite` как канонический локальный источник активного календаря.
4. Затем строим bootstrap и `Splash`, чтобы запуск приложения сразу работал от реального потока загрузки данных.
5. Дальше выводим `Year` как основной обзорный экран, вокруг которого собирается базовый пользовательский сценарий.
6. После этого добавляем `Month detail`, чтобы пользователь мог проваливаться из годового обзора в детализированное представление месяца.
7. Затем подключаем `Settings` в 4 последовательных частях:
   - `Settings entry` - аккуратно встраиваем вход в экран настроек в общий пользовательский маршрут.
   - `Theme context` - поднимаем глобальный theme state и единую цветовую схему приложения.
   - `Localization` - добавляем переключение языка и перевод интерфейса.
   - `JSON import entry` - подключаем запуск импорта `JSON` из настроек как отдельную точку входа.
8. Затем добавляем `file picker` в импорт `JSON`, доводим валидацию до полного рабочего контура и вручную пробуем загрузить календарь `2025` года, чтобы протестировать поведение замены данных и обработки ошибок.
9. После стабилизации основного сценария добавляем анимации, визуальную выразительность и прикладную оптимизацию, чтобы приложение ощущалось живым, аккуратным и отзывчивым без лишней декоративности. Делим на последовательные части:
   - `Visual polish` - выравниваем типографику, отступы и состояния компонентов относительно **уже принятого UI в приложении**; `pencil-new.pen` подключаем только как подсказку, если совпадает с выбранным направлением, без отката продуктового вида к макету.
   - `Motion and feedback` - добавляем уместные анимации и микровзаимодействия: переходы между экранами, появление контента, отклик на нажатия; избегаем декоративного «шума».
   - `Performance pass` - замеряем узкие места на целевых устройствах и точечно ускоряем: списки и сетки дней, лишние перерисовки, навигация; правки не смешиваем с первичной визуальной правкой, чтобы проще отлавливать регрессии.
10. Затем собираем flow импорта как набор отдельных экранных задач с понятными состояниями, подтверждением замены данных и прозрачной обработкой ошибок.
11. После этого добавляем внутренний push в конце года с напоминанием о новом JSON-шаблоне на следующий год и ссылкой из настроек на Telegram-бота, где этот файл можно скачать.
12. Затем закладываем прогон приложения от заказчика и вносим важные правки, чтобы сверить продукт с реальным сценарием использования и уточнить приоритетные доработки.
13. В финале доводим UX, edge cases и тестовое покрытие до цельного production-ready состояния.
...
Цель: довести интерфейс до состояния, согласованного с **уже принятым UI в приложении** и с правилами Android UI (макеты Pencil не задают обязательный целевой вид).
...
Во всех задачах `P1`–`Pn` строка **«Опора на дизайн»** с узлами `pencil-new.pen` — это **вспомогательная карта** для поиска экрана в макете; визуальный канон по-прежнему **текущий UI в приложении** (см. раздел «Дизайн-опора»).
